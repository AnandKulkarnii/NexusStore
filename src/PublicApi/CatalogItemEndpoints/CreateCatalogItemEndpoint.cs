...
using NexusStore.ApplicationCore.Exceptions;
using NexusStore.ApplicationCore.Interfaces;
using NexusStore.ApplicationCore.Specifications;
...
public class CreateCatalogItemEndpoint : IEndpoint<IResult, CreateCatalogItemRequest, IRepository<CatalogItem>>
{
    ...
    public void AddRoute(IEndpointRouteBuilder app)
    {
        app.MapPost("api/catalog-items",
            [Authorize(Roles = "Administrators", AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)] async
    ...
    public async Task<IResult> HandleAsync(CreateCatalogItemRequest request, IRepository<CatalogItem> itemRepository)
    {
        ...
        if (newItem.Id != 0)
        {
            ...
            //  pointed out by the community. More info in this issue: https://github.com/dotnet-architecture/NexusStore/issues/537 
